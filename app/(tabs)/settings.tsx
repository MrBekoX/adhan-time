...
export default function Settings() {
  ...
  const onSoundChange = async (nextSound: 'default' | 'notification'): Promise<void> => {
    setSound(nextSound);
    if (!location) return;
    try {
      ...
      >
        ...
        <Section title={t('screens.settings.sound')} ordinal="iii">
          <Row>
            ...
            />
            <Chip
              active={sound === 'notification'}
              label={t('screens.settings.soundNotification')}
              shortLabel="♪"
              onPress={() => void onSoundChange('notification')}
            />
          </Row>
        </Section>
